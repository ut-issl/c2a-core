...
static TF_TLM_FUNC_ACK Tlm_AOBC_HK_(uint8_t* packet, uint16_t* len, uint16_t max_len)
{
  ...
  TF_copy_u32(&packet[60], PL_count_executed_nodes(&PH_tl_cmd_list[TLCD_ID_FROM_GS]));
  TF_copy_u8(&packet[64], (uint8_t)(PL_count_active_nodes(&PH_tl_cmd_list[TLCD_ID_FROM_GS])));
  TF_copy_u32(&packet[65], timeline_command_dispatcher->dispatcher[TLCD_ID_FROM_GS].prev.time.total_cycle);
  TF_copy_u16(&packet[69], (uint16_t)(timeline_command_dispatcher->dispatcher[TLCD_ID_FROM_GS].prev.code));
  TF_copy_i8(&packet[71], (int8_t)(timeline_command_dispatcher->dispatcher[TLCD_ID_FROM_GS].prev.sts));
  TF_copy_u32(&packet[72], timeline_command_dispatcher->dispatcher[TLCD_ID_FROM_GS].prev_err.time.total_cycle);
  TF_copy_u16(&packet[76], (uint16_t)(timeline_command_dispatcher->dispatcher[TLCD_ID_FROM_GS].prev_err.code));
  TF_copy_i8(&packet[78], (int8_t)(timeline_command_dispatcher->dispatcher[TLCD_ID_FROM_GS].prev_err.sts));
  TF_copy_u32(&packet[79], (PL_is_empty(&(PH_tl_cmd_list[TLCD_ID_FROM_GS])) ? 0 : (uint32_t)CCP_get_ti((const CommonCmdPacket*)(PL_get_head(&(PH_tl_cmd_list[TLCD_ID_FROM_GS]))->packet))));
  TF_copy_u16(&packet[83], (uint16_t)(PL_is_empty(&(PH_tl_cmd_list[TLCD_ID_FROM_GS])) ? 0 : CCP_get_id((const CommonCmdPacket*)(PL_get_head(&(PH_tl_cmd_list[TLCD_ID_FROM_GS]))->packet))));
  TF_copy_u32(&packet[85], PL_count_executed_nodes(&PH_tl_cmd_list[TLCD_ID_DEPLOY_BC]));
  TF_copy_u8(&packet[89], (uint8_t)(PL_count_active_nodes(&PH_tl_cmd_list[TLCD_ID_DEPLOY_BC])));
  TF_copy_u32(&packet[90], PL_count_executed_nodes(&PH_tl_cmd_list[TLCD_ID_DEPLOY_TLM]));
  TF_copy_u8(&packet[94], (uint8_t)(PL_count_active_nodes(&PH_tl_cmd_list[TLCD_ID_DEPLOY_TLM])));
  TF_copy_u8(&packet[95], (uint8_t)(((uint8_t)timeline_command_dispatcher->dispatcher[TLCD_ID_FROM_GS].stop_on_error << 7 & 0x80) | ((uint8_t)timeline_command_dispatcher->dispatcher[TLCD_ID_FROM_GS].lockout << 6 & 0x40) | ((uint8_t)timeline_command_dispatcher->dispatcher[TLCD_ID_DEPLOY_BC].stop_on_error << 5 & 0x20) | ((uint8_t)timeline_command_dispatcher->dispatcher[TLCD_ID_DEPLOY_BC].lockout << 4 & 0x10) | ((uint8_t)timeline_command_dispatcher->dispatcher[TLCD_ID_DEPLOY_TLM].stop_on_error << 3 & 0x08) | ((uint8_t)timeline_command_dispatcher->dispatcher[TLCD_ID_DEPLOY_TLM].lockout << 2 & 0x04) | (0 << 1 & 0x02) | (0 & 0x01) ));
  TF_copy_u8(&packet[96], (uint8_t)((wdt_config->is_wdt_enable << 7 & 0x80) | (wdt_config->is_clear_enable << 6 & 0x40) | 0x00));
  TF_copy_u16(&packet[97], block_command_table->pos.block);
  TF_copy_u8(&packet[99], (uint8_t)(block_command_table->pos.cmd));
  TF_copy_u32(&packet[100], ((block_command_table->pos.cmd == 0) ? 0 : (uint32_t)BCT_get_ti(block_command_table->pos.block, (uint8_t)(block_command_table->pos.cmd-1))));
  TF_copy_u16(&packet[104], (uint16_t)((block_command_table->pos.cmd == 0) ? 0 : BCT_get_id(block_command_table->pos.block, (uint8_t)(block_command_table->pos.cmd-1))));
  TF_copy_u32(&packet[106], event_logger->statistics.record_counter_total);
  TF_copy_u16(&packet[110], (uint16_t)event_logger->statistics.record_counters[EL_ERROR_LEVEL_HIGH]);
  TF_copy_u16(&packet[112], (uint16_t)event_logger->statistics.record_counters[EL_ERROR_LEVEL_MIDDLE]);
  TF_copy_u16(&packet[114], (uint16_t)event_logger->statistics.record_counters[EL_ERROR_LEVEL_LOW]);
  TF_copy_u16(&packet[116], (uint16_t)event_logger->statistics.record_counters[EL_ERROR_LEVEL_EL]);
  TF_copy_u16(&packet[118], (uint16_t)event_logger->statistics.record_counters[EL_ERROR_LEVEL_EH]);
  TF_copy_u8(&packet[120], (uint8_t)event_logger->latest_event.group);
  TF_copy_u32(&packet[121], event_logger->latest_event.local);
  TF_copy_u8(&packet[125], (uint8_t)event_logger->latest_event.err_level);
  TF_copy_u32(&packet[126], event_logger->latest_event.time.total_cycle);
  TF_copy_u8(&packet[130], (uint8_t)event_logger->latest_event.time.step);
  TF_copy_u32(&packet[131], event_logger->latest_event.note);
  TF_copy_u8(&packet[135], (uint8_t)EL_get_the_nth_tlog_from_the_latest(EL_ERROR_LEVEL_HIGH, 0)->group);
  TF_copy_u32(&packet[136], EL_get_the_nth_tlog_from_the_latest(EL_ERROR_LEVEL_HIGH, 0)->local);
  TF_copy_u32(&packet[140], EL_get_the_nth_tlog_from_the_latest(EL_ERROR_LEVEL_HIGH, 0)->time.total_cycle);
  TF_copy_u8(&packet[144], (uint8_t)EL_get_the_nth_tlog_from_the_latest(EL_ERROR_LEVEL_HIGH, 0)->time.step);
  TF_copy_u16(&packet[145], (uint16_t)EL_get_the_nth_tlog_from_the_latest(EL_ERROR_LEVEL_HIGH, 0)->note);
  TF_copy_u32(&packet[147], event_handler->log_table.respond_counter);
  TF_copy_u8(&packet[151], (uint8_t)(gs_command_dispatcher->error_counter));
  TF_copy_u32(&packet[152], GIT_REV_CORE_SHORT);
  TF_copy_u32(&packet[156], GIT_REV_USER_SHORT);
  TF_copy_u32(&packet[160], 0);
  TF_copy_u32(&packet[164], 0);
  TF_copy_u32(&packet[168], 0);
  TF_copy_u32(&packet[172], 0);
  TF_copy_u32(&packet[176], 0);
  TF_copy_u32(&packet[180], 0);
  ...
  TF_copy_u32(&packet[340], 1);
  TF_copy_u32(&packet[344], 2);
  TF_copy_u32(&packet[348], 3);
  TF_copy_u32(&packet[352], 4);
  TF_copy_u32(&packet[356], 5);
  TF_copy_u32(&packet[360], 6);
  TF_copy_u32(&packet[364], 7);
  TF_copy_u32(&packet[368], 8);
  TF_copy_u32(&packet[372], 9);
  TF_copy_u32(&packet[376], 10);
  TF_copy_u32(&packet[380], 11);
  TF_copy_u32(&packet[384], 12);
  TF_copy_u32(&packet[388], 13);
  TF_copy_u32(&packet[392], 14);
  TF_copy_u32(&packet[396], 15);
  TF_copy_u32(&packet[400], 16);
  TF_copy_u32(&packet[404], 17);
  TF_copy_u32(&packet[408], 18);
  TF_copy_u32(&packet[412], 19);
  TF_copy_u32(&packet[416], 20);
  TF_copy_u32(&packet[420], 21);
  TF_copy_u32(&packet[424], 22);
  TF_copy_u32(&packet[428], 23);
#endif
